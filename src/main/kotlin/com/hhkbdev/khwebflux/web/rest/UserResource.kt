...
import com.hhkbdev.khwebflux.config.LOGIN_REGEX
...
import com.hhkbdev.khwebflux.service.MailService
import com.hhkbdev.khwebflux.service.UserService
import com.hhkbdev.khwebflux.service.dto.AdminUserDTO
import com.hhkbdev.khwebflux.web.rest.errors.BadRequestAlertException
import com.hhkbdev.khwebflux.web.rest.errors.EmailAlreadyUsedException
import com.hhkbdev.khwebflux.web.rest.errors.LoginAlreadyUsedException
 ...
 *
 * This class accesses the {@link com.hhkbdev.khwebflux.domain.User} entity, and needs to fetch its collection of authorities.
 *
...
class UserResource(
            ...
            .map { user ->
                try {
                    ResponseEntity.created(URI("/api/admin/users/${user.login}"))
                        .headers(HeaderUtil.createAlert(applicationName, "A user is created with identifier " + user.login, user.login))
                        .body(user)
    ...
    fun updateUser(@Valid @RequestBody userDTO: AdminUserDTO): Mono<ResponseEntity<AdminUserDTO>> {
        ...
        return userRepository.findOneByEmailIgnoreCase(userDTO.email!!)
            ...
            .map { user ->
                ResponseEntity.ok()
                    .headers(HeaderUtil.createAlert(applicationName, "A user is updated with identifier " + userDTO.login, userDTO.login))
                    .body(user)
            }
    }
    ...
    fun deleteUser(@PathVariable @Pattern(regexp = LOGIN_REGEX) login: String): Mono<ResponseEntity<Void>> {
        ...
        return userService.deleteUser(login)
            .then(Mono.just(ResponseEntity.noContent().headers(HeaderUtil.createAlert(applicationName, "A user is deleted with identifier " + login, login)).build()))
    }
}
