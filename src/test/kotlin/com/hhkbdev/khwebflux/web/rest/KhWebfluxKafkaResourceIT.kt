...
import com.hhkbdev.khwebflux.config.KafkaSseConsumer
import com.hhkbdev.khwebflux.config.KafkaSseProducer
...
@WithMockUser
@EmbeddedKafka
class KhWebfluxKafkaResourceIT {
    ...
    fun producesMessages() {
        client.post().uri("/api/kh-webflux-kafka/publish?message=value-produce")
            .exchange()
            .expectStatus()
            .isNoContent
    ...
    fun consumesMessages() {
        ...
        val value = client
            .get()
            .uri("/api/kh-webflux-kafka/consume")
